...
public class RoomsPlacer : MonoBehaviour
{
    ...
    private void PlaceOneRoom()
    {
        ...
        while (limit-- > 0)
        {
            ...
            {
                newRoom.transform.position = new Vector3(position.x - 5, position.y - 5, 0) * 33;
    ...
    private bool ConnectToSomething(Room room, Vector2Int p)
    {
        ...
        if (selectedDirection == Vector2Int.up)
        {
            ...
            room.HallU.SetActive(true);
            selectedRoom.DoorD.SetActive(false);
            selectedRoom.HallD.SetActive(true);
        }
        else if (selectedDirection == Vector2Int.down)
        {
            room.DoorD.SetActive(false);
            room.HallD.SetActive(true);
            selectedRoom.DoorU.SetActive(false);
            selectedRoom.HallU.SetActive(true);
        }
        else if (selectedDirection == Vector2Int.right)
        {
            room.DoorR.SetActive(false);
            room.HallR.SetActive(true);
            selectedRoom.DoorL.SetActive(false);
            selectedRoom.HallL.SetActive(true);
        }
        else if (selectedDirection == Vector2Int.left)
        {
            room.DoorL.SetActive(false);
            room.HallL.SetActive(true);
            selectedRoom.DoorR.SetActive(false);
            selectedRoom.HallR.SetActive(true);
        }
